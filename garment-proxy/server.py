        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                if resp.status_code == 200:
                    ...
                    return await poll_and_return_job(job_id, kaggle_job_id)
                else:
                    backend_detail = ""
                    try:
                    ...
                    print(f"Kaggle returned {resp.status_code}: {backend_detail}")
...
async def poll_and_return_job(job_id: str, kaggle_job_id: str, max_wait: int = 300) -> StreamingResponse:
    start = time.time()
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                if resp.status_code == 200 and resp.headers.get("content-type", "").startswith("application/zip"):
                    ...
                    await sb_update_job(job_id, {"status": "failed", "error_message": error_data.get("error", "unknown")})
                    raise HTTPException(502, f"Backend processing failed: {error_data.get('error', 'unknown')}")
        except httpx.TimeoutException:
            pass
        except HTTPException:
            raise
